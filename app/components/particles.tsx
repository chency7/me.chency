...
const vertexShader = `
  ...
  void main() {
    vec2 pos = position;
    vec2 mouse = mousePos;  // mousePos 已经在传入时处理过 dpr
    vec2 normPos = pos;     // position 已经是设备像素坐标
    
    float dist = distance(normPos, mouse) / dpr;  // 转换回CSS像素计算距离
    float force = max(0.0, 1.0 - dist * 0.01);    // 调整力的衰减范围
    force = force * force;
    
    vec2 dir = normalize(mouse - normPos);
    pos += dir * force * 50.0 * dpr;  // 根据 dpr 缩放力的大小
